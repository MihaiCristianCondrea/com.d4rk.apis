/**
 * @file English with Lidia workspace page entrypoint.
 */

/* Change Rationale: This route entry moved from legacy `src/app/**` into `src/pages/**`
 * to establish page ownership boundaries while preserving current runtime behavior. */
...
import { createGithubWizardController } from '@/shared/workspaces/data/services/github-wizard-controller.js';
import { renderWorkspaceDashboards } from '@/widgets/workspace-dashboard/workspace-dashboard.ce.js';
import { initBuilderRemoteControls } from '@/shared/workspaces/ui/remote-controls.js';
...
(function (global) {
    ...
    function initEnglishWorkspace() {
        ...
        // Change Rationale: Hydrate the shared workspace dashboard and remote partials before caching DOM nodes so
        // cross-workspace IDs exist for controller bindings, presets, and GitHub launch hooks.
        renderWorkspaceDashboards(workspaceRoot);
        initBuilderRemoteControls(workspaceRoot);
    ...
    function initHomeBuilder() {
        ...
        const validationStatus = document.getElementById('englishHomeValidation');
        ...
        function importJson(text) {
            try {
                ...
                state.cards = cards.map((raw) => ({
                    ...
                    lesson_thumbnail_image_url: utils.trimString(toStringValue(raw.lesson_thumbnail_image_url)),
                    lesson_deep_link_path: utils.trimString(toStringValue(raw.lesson_deep_link_path)),
        ...
        function sanitizeHomeCard(card) {
            const payload = {};
            const lessonId = utils.trimString(card.lesson_id ?? '');
            if (lessonId) payload.lesson_id = lessonId;
            const lessonType = utils.trimString(card.lesson_type ?? '');
            if (lessonType) payload.lesson_type = lessonType;
            const lessonTitle = utils.trimString(card.lesson_title ?? '');
            if (lessonTitle) payload.lesson_title = lessonTitle;
            const thumb = utils.trimString(card.lesson_thumbnail_image_url ?? '');
            if (thumb) payload.lesson_thumbnail_image_url = thumb;
            const deepLink = utils.trimString(card.lesson_deep_link_path ?? '');
            if (deepLink) payload.lesson_deep_link_path = deepLink;
            card.customFields
    ...
    function initLessonBuilder() {
        ...
        const validationStatus = document.getElementById('englishLessonValidation');
