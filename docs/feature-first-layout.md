...
## Canonical structure
- Core infrastructure stays in `src/core/` (router, styles, services, shared UI).
<!-- Change Rationale: Update the example feature path to match the current githubtools folder naming. -->
- User-facing modules live in `src/app/<feature>/` with subfolders `data/`, `domain/`, and `ui/` (for example, `app/home`, `app/githubtools`, `app/workspaces/app-toolkit`).
- Workspace folders match API slugs (for example, `app/workspaces/faq` instead of `faqs`).

## Import conventions
- Prefer importing from `@/app/...` for feature code. Compatibility barrels remain under `src/app/...` to keep historic imports working during the transition.
- Router imports should target `core/ui/router/`. A thin facade exists at `src/core/ui/router/` solely for backwards compatibility.
